...
function finalizeFrame(dots, lines, rMin = 0.3) {
  const visible = []
  for (const d of dots) { if ((d.a ?? 1) < 0.02) continue; d.r = Math.max(rMin, d.r); visible.push(d) }
...
function scaleCounts(opts, scale) {
  const out = { ...opts }, done = new Set()
  const rt = Math.sqrt(scale)
  for (const [a, b] of COUNT_PAIRS) {
    const va = out[a], vb = out[b]
    if (va != null && vb != null && !done.has(a) && !done.has(b)) {
      out[a] = Math.max(2, Math.round(va * rt)); out[b] = Math.max(2, Math.round(vb * rt))
      done.add(a); done.add(b)
    }
  }
  for (const k of COUNT_KEYS) { const v = out[k]; if (v != null && v !== 0 && !done.has(k)) out[k] = Math.max(1, Math.round(v * scale)) }
  if (out.iconD != null) out.iconD = Math.max(0.02, out.iconD * scale)
  return out
}
function scaleRadii(opts, scale) {
  const out = { ...opts }
  for (const k of RADIUS_KEYS) { const v = out[k]; if (v != null) out[k] = v * scale }
...
const PRESETS = {
  orbits: { 20: { speed: 3.9, count: 0.238, size: 2.4 }, 64: { speed: 1.885, count: 1, size: 1 } },
  globe:  { 20: { speed: 2.665, count: 0.105, size: 1.75, extra: { scanMul: 4.335, dimBase: 0.45 } }, 64: { speed: 2.015, count: 0.42, size: 1.15, extra: { scanMul: 4.08, dimBase: 0.45 } } },
  rubik:  { 20: { speed: 1.95, count: 0.088, size: 1.9 }, 64: { speed: 1.82, count: 0.35, size: 1.05 } },
  wave:   { 20: { speed: 3.998, count: 0.105, size: 1.6 }, 64: { speed: 4.388, count: 0.341, size: 1 } },
  web:    { 20: { speed: 6.63, count: 0.25, size: 1.52 }, 64: { speed: 3.315, count: 1.35, size: 0.95 } },
  braid:  { 20: { speed: 2.75, count: 0.1125, size: 1.36 }, 64: { speed: 1.625, count: 0.5, size: 1 } },
  ribbon: { 20: { speed: 3.12, count: 0.051, size: 1.073, extra: { spin: 0, bandMul: 4.94, wobMul: 1 } }, 64: { speed: 2.34, count: 0.25, size: 0.85, extra: { spin: 0, bandMul: 3.9, wobMul: 1 } } },
  ring:   { 20: { speed: 3.78, count: 0.028, size: 1.622, extra: { spin: 0, bandMul: 3.968, wobMul: 0.565 } }, 64: { speed: 3.24, count: 0.25, size: 0.956, extra: { spin: 0, bandMul: 3.627, wobMul: 0.368 } } },
  morph:  { 20: { speed: 2.08, count: 0.53, size: 1.011, extra: { spread: 1.45 } }, 64: { speed: 2.405, count: 0.702, size: 0.395, extra: { spread: 1.45 } } }
}
const STATE_TO_MODE = {
...
function frameGlobe(size, t, o) {
  ...
  for (let li = 0; li <= latRings; li++) {
    ...
    for (let lj = 0; lj < lonCount; lj++) {
      ...
      dots.push({ x: px, y: py, z, r: ((o.rBase ?? 0.6) + (o.rDepth ?? 1.7) * depth + (o.rBoost ?? 1) * boost) * rs, white: (o.inkFar ?? 0.62) - (o.inkSpan ?? 0.54) * depth, a: dimBase + (1 - dimBase) * Math.min(1, boost) })
    }
  }
  return finalizeFrame(dots, [], o.rMin)
}
...
function frameRubik(size, t, o) {
  ...
  for (let li = 0; li <= latRings; li++) {
    ...
    for (let lj = 0; lj < lonCount; lj++) {
      ...
      const depth = (zr + 1) / 2
      dots.push({ x: px, y: py, z: zr, r: ((o.rBase ?? 0.6) + (o.rDepth ?? 1.7) * depth + (inActive ? (o.rActive ?? 0.3) : 0)) * rs, white: (o.inkFar ?? 0.62) - (o.inkSpan ?? 0.54) * depth - (inActive ? 0.14 : 0) })
    }
  }
  return finalizeFrame(dots, [], o.rMin)
}
function frameWave(size, t, o) {
  ...
  for (let ri = 0; ri <= rings; ri++) {
    ...
    for (let lj = 0; lj < lonCount; lj++) {
      ...
      dots.push({ x: px, y: py, z, r: ((o.rBase ?? 0.6) + (o.rDepth ?? 1.7) * depth) * (1 + 0.4 * crest) * rs, white: 0.66 - 0.56 * depth - 0.1 * crest })
    }
  }
  return finalizeFrame(dots, [], o.rMin)
}
...
function frameBraid(size, t, o) {
  ...
  for (let s = 0; s < 3; s++) {
    ...
    for (let i = 0; i < strandN; i++) {
      ...
      dots.push({ x: px, y: py, z: zr, r: ((o.rBase ?? 1.2) + (o.rDepth ?? 1.8) * depth) * rs, white: 0.55 - 0.45 * depth, a: endFade * (0.45 + 0.55 * depth) })
    }
  }
  return finalizeFrame(dots, [], o.rMin)
}
function frameRibbon(size, t, o) {
  ...
  const spin = o.spin ?? 1, camTilt = 0.3
  ...
  for (let w = 0; w < lanes; w++) {
    ...
    for (let k = 0; k < segs; k++) {
      ...
      const depth = (zr / R + 1) / 2
      dots.push({ x: px, y: py, z: zr, r: ((o.rBase ?? 1.1) + (o.rDepth ?? 1.7) * depth) * (1 - 0.25 * edge) * rs, white: 0.52 - 0.44 * depth + 0.18 * edge, a: 0.4 + 0.6 * depth })
    }
  }
  return finalizeFrame(dots, [], o.rMin)
}
...
const morphN = d => Math.max(6, Math.round(34 * d))
function frameMorph(size, t, o) {
  const K = CYCLE.length, HOLD = 1.4, MORPH = 0.9, SEG = HOLD + MORPH
  const tc = t % (SEG * K)
...
function resolvePreset(state, size) {
  ...
  const preset = PRESETS[mode][size === 64 ? 64 : 20]
  let opts = { ...BASE[mode] }
...
function OrbCanvas({ size, stateRef }) {
  ...
  useEffect(() => {
    ...
    let dark = mq.matches
    const onDark = e => { dark = e.matches }
    mq.addEventListener('change', onDark)
    let { mode, speed, opts } = resolvePreset(stateRef.current, size)
    let raf = 0
    const loop = (now) => {
      const t = (now / 1000) * speed
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0)
      ctx.clearRect(0, 0, size, size)
      const f = FRAMES[mode](size, t, opts)
      if (f.lines.length) paintLines(ctx, f.lines, dark)
      paint(ctx, f.dots, dark)
      raf = requestAnimationFrame(loop)
    }
    raf = requestAnimationFrame(loop)
    const stateWatch = window.setInterval(() => {
      const r = resolvePreset(stateRef.current, size)
      mode = r.mode; speed = r.speed; opts = r.opts
    }, 250)
    return () => {
